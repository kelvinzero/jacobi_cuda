...
////////////////////////////////////////////////////////////////////////////////
// declaration, forward
void runCUDA( float *h_dataA, float* h_dataB, int width, int height, int passes, int threadsPerBlock, int shouldPrint, int optimized);
...
main( int argc, char** argv) 
{
   ...
   if(argc == 7){
	if(argv[5][0]=='p' && argv[6][0] =='s'){
	    optimized = 0;
	    shouldPrint = 1;
	}
	else if(argv[5][0]=='p' && argv[6][0] =='o'){
	    optimized = 1;
	    shouldPrint = 1;
	}
	else{
   ...
   if (threadsPerBlock == 0){
      ...
      runCUDA(h_dataA, h_dataB, width, height, passes, threadsPerBlock, shouldPrint, optimized);
   }
...
void runCUDA( float *h_dataA, float* h_dataB, int width, int height, int passes, int threadsPerBlock, int shouldPrint, int optimized){
   ...
   float * temp;
   if(optimized)
	   printf("Executing kernel optimized\n");
   else
	   printf("Executing kernel simple\n");
...
void usage(){
   ...
   fprintf(stderr, "usage: jacobi threadsperblock passes width height [p] [o,s]\n");
